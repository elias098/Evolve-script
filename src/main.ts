...
(function ($) {
  ...
  class Resource {
    ...
    isRoutesUnlocked() {
      return (
        this.isUnlocked() &&
        (!game.global.race["artifical"] || this !== resources.Food) &&
    ...
    getBusyWorkers(workersSource, workersCount, locArg) {
      ...
      if (workersCount > 0) {
        ...
        let usedIncome =
          totalIncome - this.calculateRateOfChange({ buy: false, all: true });
        if (usedIncome > 0) {
    ...
    get timeToFull() {
      ...
      let totalRateOfCharge = this.calculateRateOfChange({
        buy: false,
        all: true,
      });
  ...
  class Technology {
    ...
    get title() {
      ...
      if (
        def.path &&
        def.path.includes("truepath") &&
        !def.path.includes("standard")
      ) {
  ...
  class Race {
    ...
    getWeighting() {
      ...
      const checkAchievement = (baseWeight, id) => {
        weighting +=
          baseWeight * Math.max(0, starLevel - getAchievementStar(id));
        if (
          game.global.race.universe !== "micro" &&
      ...
      if (
        ...
        weighting +=
          1000 * Math.max(0, starLevel - (game.global.pillars[this.id] ?? 0));
        ...
        if (
          ...
          let genusPillar = Math.max(
            ...Object.values(races)
              .filter(
                (r) =>
                  r.id !== "custom" && (r.id !== "junker") & (r.id !== "sludge")
              )
      ...
      if (game.global.race.universe !== "micro") {
        const checkFeat = (id) => {
          weighting +=
            1 * Math.max(0, starLevel - (game.global.stats.feat[id] ?? 0));
        };
    ...
    constructor(traitName) {
      ...
      let ownerRace =
        Object.entries(game.races)
          .filter(
            ([id, race]) =>
              id !== "custom" && race.traits[traitName] !== undefined
          )
  ...
  var GrapheneManager = {
    ...
    initIndustry() {
      this._graphPlant = game.global.race["truepath"]
  ...
  var WarManager = {
    ...
    getHellReservedSoldiers() {
      ...
      if (settings.autoBuild && buildings.PitAssaultForge.isAutoBuildable()) {
        let missingRes = Object.entries(buildings.PitAssaultForge.cost).find(
  ...
  var BuildingManager = {
    ...
    managedStatePriorityList() {
      return this.statePriorityList.filter(
        (building) =>
          building.hasState() && building.autoStateEnabled && building.count > 0
      );
    },
  };
  ...
  function initialiseState() {
    ...
    Object.defineProperty(resources.Crates, "cost", {
      get: () => (isLumberRace() ? { Plywood: 10 } : { Stone: 200 }),
    });
    ...
    buildings.TauOrbitalStation.addResourceConsumption(resources.Helium_3, () =>
    ...
    buildings.TauBeltPatrolShip.addResourceConsumption(resources.Helium_3, () =>
      haveTech("isolation") ? 15 : 250
    );
    buildings.TauBeltMiningShip.addResourceConsumption(resources.Helium_3, () =>
  ...
  function updateStandAloneSettings() {
    ...
    if (settingsRaw.hasOwnProperty("genesAssembleGeneAlways")) {
      if (settingsRaw.overrides.genesAssembleGeneAlways) {
        settingsRaw.overrides.autoAssembleGene =
      ...
      if (!settingsRaw.genesAssembleGeneAlways) {
        settingsRaw.overrides.autoAssembleGene =
          settingsRaw.overrides.autoAssembleGene ?? [];
        settingsRaw.overrides.autoAssembleGene.push({
    ...
    if (state.evolutionTarget === null) {
      ...
      if (settings.userEvolutionTarget === "auto") {
        ...
        if (game.global.stats.achieve["mass_extinction"]) {
          ...
          let genusList = Object.values(races)
            .map((r) => r.genus)
    ...
    for (let i = 0; i < maxPlanets; i++) {
      ...
      if (avail.length > 0 && Math.floor(seededRandom(0, 10)) === 0) {
        ...
        if (planet.biome === "hellscape") {
          ...
          planet.orbit = Math.floor(
            seededRandom(200, planet.traits.includes("elliptical") ? 800 : 600)
          );
        }
      }
      ...
      if (
        ...
        let scaledWeightings = Object.fromEntries(
          availableJobs.map((job) => [
            job.id,
            (state.unlockedBuildings.find(
              (building) =>
                building.cost[job.resource.id] > job.resource.currentQuantity
          ...
          if (job === jobs.HellSurveyor) {
            if (jobMax[j] === undefined) {
              if (
                ...
                jobMax[j] = resources.Infernite.getBusyWorkers(
                  "job_hell_surveyor",
                  jobs.HellSurveyor.count
                );
              } else {
                jobMax[j] = Number.MAX_SAFE_INTEGER;
              }
    ...
    if (splitJobs.length > 0) {
      ...
      for (
        ...
        while (
          ...
          if (
            (b === 2 || total < jobDetails.job.breakpointEmployees(b)) &&
            !(total >= jobMax[jobDetails.index])
          ) {
  ...
  function autoAlchemy() {
    ...
    if (!resources.Crystal.isDemanded()) {
      ...
      let maxTransmute = Math.floor(
        Math.min(manaAvailable, crystalAvailable * 2)
      );
      activeList.forEach(
        (res) =>
  ...
  function autoPylon() {
    ...
    let remainingSpells = spells
      ...
      .sort(spellSorter);
    spellsLoop: while (remainingSpells.length > 0) {
  ...
  function autoSmelter() {
        ...
        for (let productionCost of fuel.cost) {
          ...
          if (resource.storageRatio < 0.8 || resource === resources.StarPower) {
            ...
            if (
              ...
              resource !== resources.StarPower
            ) {
              state.tooltips[
                "smelterFuels" + fuel.id.toLowerCase()
    ...
    for (let productionCost of steelSmeltingConsumption) {
      ...
      if (resource.storageRatio < 0.8) {
        let remainingRateOfChange =
          resource.rateOfChange + smelterSteelCount * productionCost.quantity;
        // No need to preserve minimum income when storage is full
        if (resource.storageRatio < 0.98) {
          remainingRateOfChange -= productionCost.minRateOfChange;
        }
        ...
        for (
          ...
          for (let resourceCost of production.cost) {
            ...
            if (!production.resource.isDemanded()) {
              if (!settings.useDemanded && resourceCost.resource.isDemanded()) {
                actualRequiredFactories = 0;
                state.tooltips[
                  "iFactory" + production.id
                ] += `${resourceCost.resource.name} is demanded<br>`;
                break;
              }
              if (
                ...
                state.tooltips[
                  "iFactory" + production.id
                ] += `${resourceCost.resource.name} under min materials ratio<br>`;
                break;
              }
            }
            if (resourceCost.resource.storageRatio < 0.8) {
              ...
              let rate =
                resourceCost.resource.rateOfChange + previousCost - currentCost;
              ...
              if (affordableAmount < 1) {
                state.tooltips[
                  "iFactory" + production.id
                ] += `Too low ${resourceCost.resource.name} income<br>`;
              }
          ...
          if (
            ...
            resources.Neutronium.currentQuantity <
              (game.global.race["truepath"] ? 500 : 250)
          ) {
            state.tooltips["iFactory" + production.id] += `${
              game.global.race["truepath"] ? 500 : 250
            } ${resources.Neutronium.name} reserved<br>`;
            actualRequiredFactories = 0;
          }
  ...
  function autoGraphenePlant() {
    ...
    for (let fuel of sortedFuel) {
      ...
      if (!resources.Graphene.isUseful()) {
        ...
        let rateOfChange =
          resource.rateOfChange + fuel.cost.quantity * currentFuelCount;
        if (resource.storageRatio < 0.98) {
  ...
  function autoReplicator() {
    ...
    if (priorityList.length > 0 && priorityList[0].length > 0) {
      var selectedResource = priorityList[0].sort(
        (a, b) => a.weighting - b.weighting
      )[0];
    ...
    if (game.global.race.governor.config.replicate.pow.on == false) {
      win.document
        .querySelector("#govOffice .options")
        .getElementsByClassName("tConfig")[8]
        .childNodes[1].childNodes[0].childNodes[0].click(); // Enable auto power management
    }
    if (game.global.race.governor.config.replicate.res.que) {
      win.document
        .querySelector("#govOffice .options")
        .getElementsByClassName("tConfig")[8]
        .childNodes[2].childNodes[0].childNodes[0].click(); // Disable focus queue
    }
    if (game.global.race.governor.config.replicate.res.neg) {
      win.document
        .querySelector("#govOffice .options")
        .getElementsByClassName("tConfig")[8]
        .childNodes[2].childNodes[1].childNodes[0].click(); // Disable negative focus
    }
    if (game.global.race.governor.config.replicate.res.cap) {
      win.document
        .querySelector("#govOffice .options")
        .getElementsByClassName("tConfig")[8]
        .childNodes[2].childNodes[2].childNodes[0].click(); // Disable switch on cap
    }
  }
  ...
  function isPrestigeAllowed(type) {
    return (
      ...
      !(
        settings.prestigeWaitAT &&
        game.global.settings.at > 0 &&
      ...
      if (
        ...
        if (resource.storageRatio > resource.autoSellRatio) {
          ...
          maxAllowedUnits = Math.min(
            maxAllowedUnits,
            Math.floor(
              (resource.calculateRateOfChange({ buy: false, all: true }) * 2) /
                ticksPerSecond()
            )
  ...
  function getTechConflict(tech) {
    ...
    if (
      ...
      (!settings.prestigeDemonicBomb || settings.prestigeType !== "infusion")
    ) {
        ...
        if (building === buildings.TritonLander) {
          if (buildings.TritonFOB.stateOnCount < 1) {
            ...
            let healthySquads = Math.floor(
              (WarManager.currentSoldiers - WarManager.wounded) /
                (3 * traitVal("high_pop", 0, 1))
            );
  ...
  function prioritizeDemandedResources() {
    ...
    if (SpyManager.purchaseMoney && settings.prioritizeUnify.includes("req")) {
      resources.Money.requestedQuantity = Math.max(
        resources.Money.requestedQuantity,
    ...
    if (
      ...
      for (let res in FleetManagerOuter.nextShipCost) {
        ...
        resource.requestedQuantity = Math.max(
          resource.requestedQuantity,
          FleetManagerOuter.nextShipCost[res]
        );
      }
    }
    ...
    if (
      ...
      resources.Stanene.currentQuantity <
        Math.min(
          (buildings.Alien1VitreloyPlant.stateOnCount || 1) * 6000,
          resources.Stanene.maxQuantity * 0.05
        )
    ) {
    ...
    [
      {
        type: "queue",
        noorder: "qAny",
        map: (id) => buildingIds[id] || arpaIds[id],
      },
  ...
  function getTooltipInfo(obj) {
    ...
    if (
      ...
      if (conflict) {
        notes.push(
          `Conflicts with ${conflict.obj.name} for ${conflict.res.name} (${conflict.obj.cause})`
        );
      }
    }
    ...
    if (obj === buildings.LakeCoolingTower) {
      ...
      let diff =
        (current - next) *
        buildings.LakeHarbour.stateOnCount *
        (game.global.race["emfield"] ? 1.5 : 1);
      notes.push(
  ...
  function updateOverrides() {
        ...
        try {
          ...
          if (typeof settingsRaw[key] === typeof check.ret) {
            ...
            throw `Expected type: ${typeof settingsRaw[
              key
            ]}; Override type: ${typeof check.ret}`;
          }
        } catch (error) {
          let msg = `Condition ${
            i + 1
    ...
    overrides["tickRate"] = Math.min(
      ...
      Math.max(
        1,
        Math.round((overrides["tickRate"] ?? settingsRaw["tickRate"]) * 2)
      ) / 2
    );
  ...
  function mainAutoEvolveScript() {
    ...
    const setCallback = (fn) =>
      typeof unsafeWindow !== "object" || typeof exportFunction !== "function"
        ? fn
    ...
    job_servant: {
      ...
      options: () =>
        Object.values(jobIds)
          .filter((j) => j instanceof BasicJob || j instanceof CraftingJob)
  ...
  const argMap = {
    ...
    other: (o) =>
      o === "rname"
        ? game.races[game.global.race.species].name
        : o === "tpfleet"
  ...
  function buildInputNodeForDisplay(type, options, value) {
    switch (type) {
      ...
      case "list":
        return $(`
                  <span></span>`).text(
          value.map((item) => options.list[item].name).join(", ")
        );
      default:
        return $(`
  ...
  function updateGovernmentSettingsContent(secondaryPrefix) {
    ...
    let governmentOptions = [
      ...
      ...Object.keys(GovernmentManager.Types)
  ...
  function buildEvolutionQueueItem(id) {
    ...
    if (queuedEvolution.challenge_junker || queuedEvolution.challenge_sludge) {
      raceName = queuedEvolution.challenge_junker
        ? races.junker.name
        : races.sludge.name;
      if (race) {
        raceName += ", ";
  ...
  function buildTriggerSettingsColumn(trigger) {
    ...
    let deleteTriggerButton = $(
      '<a class="button is-dark is-small" style="width: 26px; height: 26px"><span>X</span></a>'
    );
    ...
    let theology2Options = [
      {
        val: "auto",
        label: "Script Managed",
        hint: "Picks Deify for Ascension, Demonic Infusion, AI Apocalypse, Terraform, Matrix, Retirement and Eden prestiges, or Study for others prestiges",
      },
      {
        val: "tech-study",
  ...
  function updateJobSettingsContent() {
    ...
    addSettingsToggle(
      ...
      "jobSetDefault",
      "Set default job",
      "Automatically sets the default job in order of Quarry Worker -> Lumberjack -> Crystal Miner -> Scavenger -> Farmer -> Hunter -> Unemployed"
    );
    addSettingsToggle(
      currentNode,
      "jobManageServants",
  ...
  function updateUI() {
    ...
    if (scriptNode.length === 0) {
      ...
      createSettingToggle(
        togglesNode,
        "autoAssembleGene",
        "Automatically assembles genes only when your knowledge is at max."
      );
      createSettingToggle(
        togglesNode,
        "autoMinorTrait",
        "Purchase minor traits using genes according to their weighting settings. Also manages Mimic genus."
      );
      createSettingToggle(
        togglesNode,
        "autoMutateTraits",
        "Mutate in or out major and genus traits. WARNING: This will spend spend Plasmids and Anti-Plasmids."
      );
      createSettingToggle(
        togglesNode,
        "autoEject",
